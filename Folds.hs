...
-}
{-
  Daca transformarea este de tip Translation, atunci se afiseaza "+(tx,ty)".
  Daca transformarea este de tip Scaling, atunci se afiseaza "*<f>".
  Daca transformarea este de tip Combine, atunci se afiseaza lista Haskell a
afisarilor transformarilor pentru fiecare transformare in parte.
-}
    ...
    * Funcția replicate permite construirea unei liste prin repetarea
    ...
    +
      ...
      *
        ...
        Rectangle 4.0 5.0
-}
-- Apelez functia ajutatoare de la nivelul de indentare 0
instance Show RegionAST where
    show = showRegionAST 0

{- 
  Functie ajutatoare care primeste ca parametru nivelul de indentare si un RegionAST
si returneaza un String care reprezinta regiunea sub forma de string.
  Stringul returnat este format din multiple randuri, fiecare rand reprezentand 
un nod din arbore.
  Pentru fiecare nod din arbore, se adauga un numar de spatii egal cu 2 * nivelul
pentru a respecta indentarea.
  Daca nodul este de tip FromPoints, Rectangle sau Circle, atunci se concateneaza
indentarea cu reprezentarea sub forma de string a nodului.
  Daca nodul este de tip Complement, Union sau Intersection atunci se concateneaza
indentarea cu caracterul corespunzator nodului, se adauga un rand nou si se apeleaza
functia recursiv pentru fiecare subregiune.
  Daca nodul este de tip Transform, atunci se concateneaza indentarea cu reprezentarea
sub forma de string a transformarii, se adauga un rand nou si apoi se apeleaza functia
recursiv pentru subregiune.
-}
showRegionAST :: Int -> RegionAST -> String
showRegionAST level (R region) =
    replicate (2 * level) ' ' ++ case region of
        FromPoints points -> "FromPoints " ++ show points
        Rectangle width height -> "Rectangle " ++ show width ++ " " ++ show height
        Circle radius -> "Circle " ++ show radius
        Complement region' -> "~\n" ++ showRegionAST (level + 1) region'
        Union region1 region2 -> "+\n" ++ showRegionAST (level + 1) region1 ++ "\n" ++ showRegionAST (level + 1) region2
...
instance Num RegionAST where
    -- Functia fromInteger primeste un numar intreg si intoarce regiunea care
    -- include doar punctul (n, n) folosind functia de constructie fromPoints
...
{-
  Daca transformarea este de tip Translation sau Scaling, atunci se intoarce
transformarea fara a aplica functia f.
  Daca transformarea este de tip Combine, atunci se aplica functia f pe fiecare
transformare din lista de transformari.
-}
...
{-
  Daca regiunea este de tip FromPoints, Rectangle sau Circle, atunci se intoarce
regiunea fara a aplica functia f.
  Daca regiunea este de tip Complement, Union sau Intersection, atunci se aplica
functia f pe fiecare subregiune.
  Daca regiunea este de tip Transform, atunci se aplica functia f doar pe subregiune.
-}
    fmap f region =
      case region of
...
-}
-- Se aplica combiner-ul pe rezultatul obtinut prin aplicarea recursiva a 
-- foldTransformationAST pe fiecare transformare din lista de transformari.
foldTransformationAST :: TransformationCombiner a -> TransformationAST -> a
foldTransformationAST f (T transformation) =
    f (fmap (foldTransformationAST f) transformation)

-- Se aplica combiner-ul pe rezultatul obtinut prin aplicarea recursiva a
-- foldRegionAST pe fiecare subregiune din regiune.
foldRegionAST :: RegionCombiner a -> RegionAST -> a
foldRegionAST f (R region) =
    f (fmap (foldRegionAST f) region)
...
{-
  Aceeasi implementare ca in etapa 2, doar ca se foloseste foldTransformationAST
pentru a reduce subarborii la un singur tip Transformation.
  Astfel pe cazul de combinare a transformarilor se aplica combiner-ul pe
lista de transformari.
-}
...
{-
  Daca transformarea este de tip Translation sau Scaling, atunci se intoarce 1.
  Daca transformarea este de tip Combine, atunci se intoarce suma transformarilor
din lista de transformari deja calculate.
-}
...
{-
    *** TODO ***
    ...
    Răspuns: In cazul in care dorim sa accesam doar o componenta a perechii, nu
    este necesar sa evaluam si cealalta componenta. Astfel, putem sa evaluam doar
    componenta dorita fara a evalua si cealalta componenta.
...
{-
  Daca regiunea este de tip FromPoints, Rectangle sau Circle, atunci se intoarce
(1, 0). => Se adauga o regiune elementara si nu se adauga nicio transformare.
  Daca regiunea este de tip Complement, atunci se intoarce perechea de regiuni
si transformari elementare deja calculata.
  Daca regiunea este de tip Union sau Intersection, atunci se intoarce perechea
de regiuni si transformari elementare rezultate din adunare.
  Daca regiunea este de tip Transform, atunci se intoarce perechea formata din
numarul de regiuni deja calculate si numarul de transformari elementare calculate
pana in punctul respectiv la care se adauga numarul de transformari elementare
calculate pentru transformarea curenta, folosind functia basicTransformationCount,
  ...
  where
    ...
    combiner region =
      case region of
        ...
        Circle _ -> (1, 0)
        Complement (r, t) -> (r, t)
        Union (r1, t1) (r2, t2) -> (r1 + r2, t1 + t2)
        Intersection (r1, t1) (r2, t2) -> (r1 + r2, t1 + t2)
        Transform transformations (r, t) -> (r, t + basicTransformationCount transformations)
{-
    *** TODO ***
